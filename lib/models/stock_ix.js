...
(function() {
  ...
  StockIX = (function(_super) {
    ...
    StockIX.prototype.analyze_data = function(cursor, raw_buf) {
      var buf, buyAmount, buyBids, contract, data, feature_price, holding, i, latest_deal, m_fAmount, m_fHigh, m_fLastClose, m_fLow, m_fNewPrice, m_fOpen, m_fVolume, market, result, sellAmount, sellBids, time_t, total_deal, val;
    ...
    StockIX.prototype.saveToDb = function(time, ticker, fLastClose, fOpen, fHigh, fLow, fNewPrice, fVolume, fAmount) {
      ...
      if (corresponding_key) {
        ...
        console.log(key);
        return redis.HMSET(key, "t", time, "close", fLastClose, "open", fOpen, "high", fHigh, "low", fLow, "current", fNewPrice, "volume", fVolume, "amount", fAmount, function(err, result) {
          console.log("[IX] [" + key + "] " + result);
          if (err) {
            return console.error("[IX] update " + corresponding_key + " failed for " + err + ".");
          }
        });
      }
    };
